      ...
      "zgsp26.conf.request.confrequestapp.ext.controller.RequestObjectExt",
      {
        ...
        onOpenConfig: async function () {
          try {
            ...
            const sConfName  = oData.ConfName || oUrlParams.ConfName || "";
            const sModuleId  = oData.ModuleId || oUrlParams.ModuleId || "";
            const sStatus    = oData.Status   || "D";
            const sEnvId     = oData.EnvId    || "DEV";
            const sTargetCds = oData.TargetCds || oUrlParams.TargetCds || "";
            ...
            const sMode    = localStorage.getItem("conf-mng-nav-mode") ||
              (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1"
                ? "local" : "deploy");
            const oEntry   = APP_PATH_MAP[sTargetCds] || APP_PATH_MAP["ZI_MM_ROUTE_CONF"];
            const sPath    = oEntry[sMode] || oEntry.deploy;
            const sAppBase = sMode === "deploy" ? window.location.origin + sPath : sPath;
